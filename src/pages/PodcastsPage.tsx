...
const PodcastsPage: React.FC = () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    const formatted = data.map((p: any) => ({
                        ...
                        host: p.host || p.guestName || "Convidado Especial",
                        category: p.category || "Estratégia",
                        img: p.coverImage || "https://images.unsplash.com/photo-1590602847861-f357a9332bbc?q=80&w=2070&auto=format&fit=crop",
                        format: p.format ? (p.format.toLowerCase() === 'video' ? 'video' : 'audio') : (p.videoUrl ? 'video' : 'audio')
                    }));
    ...
    return (
        <div style={{ backgroundColor: '#10171f', minHeight: '100vh', color: '#fff', fontFamily: 'Inter, sans-serif', overflowX: 'hidden' }}>
            ...
            <section style={{
                ...
                <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                    ...
                    >
                        ...
                        <p style={{ color: '#888', fontSize: isMobile ? '15px' : '22px', maxWidth: '700px', margin: isMobile ? '0 auto 30px' : '0 auto 50px', lineHeight: 1.4 }}>
                            Conversas profundas com quem está a construir o futuro do mercado em África. Sem filtros, apenas estratégia pura.
                        </p>

                        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'center', alignItems: 'center', gap: '15px', width: '100%', maxWidth: '400px', margin: '0 auto' }}>
                            <button style={{ 
                                ...
                                gap: '12px', 
                                boxShadow: '0 20px 40px rgba(0,17,253,0.3)' 
                            }}>
            ...
            {/* Content Section */}
            <section style={{ padding: isMobile ? '40px 0' : '80px 0', position: 'relative', zIndex: 1 }}>
                <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 20px' }}>

                    {/* Format Toggle & Search */}
                    <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', gap: isMobile ? '20px' : '40px', marginBottom: '30px' }}>
                        <div style={{ 
                            display: 'flex', 
                    ...
                    {/* Category Tabs */}
                    <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '10px', scrollbarWidth: 'none', marginBottom: isMobile ? '30px' : '60px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        {categories.map(cat => (
                            <button
                                key={cat}
                    ...
                    <div style={{
                        ...
                        <AnimatePresence mode='popLayout'>
                            {filteredEpisodes.map((ep, idx) => (
                                ...
                                >
                                    <div style={{ position: 'relative', height: isMobile ? '220px' : '300px', width: '100%' }}>
                                    ...
                                    <div style={{ padding: isMobile ? '30px 20px' : '40px 30px' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '15px' }}>
                                            <Calendar size={12} color="#555" />
                                            <span style={{ fontSize: '12px', color: '#555', fontWeight: 700 }}>{ep.date}</span>
                                        </div>
                                        <h3 style={{ fontSize: isMobile ? '20px' : '24px', fontWeight: 900, textTransform: 'uppercase', lineHeight: 1.1, marginBottom: '15px', color: '#fff' }}>{ep.title}</h3>
                                        <p style={{ fontSize: '14px', color: '#888', lineHeight: 1.6, marginBottom: '25px', minHeight: isMobile ? 'auto' : '67px' }}>{ep.description}</p>

                                        <div style={{ paddingTop: '25px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: '15px' }}>
                                            <div style={{ width: '36px', height: '36px', borderRadius: '50%', backgroundColor: '#10171f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Mic size={16} color="#0011fd" />
                                            </div>
                                            <div>
            ...
            {/* Newsletter Section */}
            <section style={{ padding: isMobile ? '60px 20px' : '120px 20px', borderTop: '1px solid rgba(255,255,255,0.05)', backgroundColor: 'rgba(0,17,253,0.02)' }}>
                <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
                    ...
                    <h2 style={{ fontSize: isMobile ? '32px' : '56px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: isMobile ? '-1px' : '-2px', marginBottom: '20px' }}>Subscreve o Feed</h2>
                    <p style={{ color: '#888', fontSize: isMobile ? '15px' : '18px', marginBottom: isMobile ? '30px' : '40px' }}>Recebe os destaques de cada episódio e recursos extras diretamente no teu e-mail.</p>
                    <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '15px', justifyContent: 'center', alignItems: 'center', width: '100%', maxWidth: '500px', margin: '0 auto' }}>
                        <input
                            type="email"
